...
Our results dissociate the two components of a value harness. The *content* of the harness,
...
ever beat it. Our data therefore do not show that Buddhist content improves moral
judgment; they show that a reflective procedure can, regardless of the
framing under which it is executed. The vows' contribution is
directional, and the benchmarks' response to that direction constitutes
the second finding.
...
improvement, and returns their sum. The scope of this claim is limited: it concerns how the three
crowd-labelled benchmarks studied here respond to framework shifts,
not the usefulness of such benchmarks in general. For screening
clearly harmful behaviour, where value-loading changed nothing
(Section 5.3), a single label is an appropriate instrument. The
difficulty is confined to comparative claims about values, where a
difference in framework and a difference in competence arrive as the
same number. Two results illustrate the point: the same vow loop that loses two to eight points against US
...
These results might be read as showing that "Buddhist AI disagrees
with Western benchmarks." That reading is too strong, for two reasons. First, the
...
baseline. That comparison is a direct extension of this work; the present
results establish its motivating phenomenon: value-loading moves
models in directions that single-label benchmarks of this kind can only
register as error.

## 6.4 Implications for value alignment

The content/procedure dissociation corresponds to a distinction the
...
on pluralistic alignment (Sorensen et al., 2024). A model that
enumerates affected parties, audits itself, and entertains dissent
before answering generates the perspectives a pluralistic response
must survey; a model with values fixed in its system prompt occupies a
single point in value space. Where alignment must serve multiple
moralities, the procedure a model executes appears to be the more
effective lever than the values it is instructed to hold.

## 6.5 Practical guidance

For practitioners who value-load open-weight models, the results
support four recommendations. Stating values in the system prompt should not, by itself, be expected
...
We asked whether the Four Great Bodhisattva Vows could improve the
moral judgment of locally deployed language models. The experiments
returned a more differentiated answer: value content and reflective
procedure do different work. Stated as a system prompt, the vows
...
Three implications follow. For value-loading practice, procedures
rather than value statements are the effective unit of intervention.
For alignment research, the results favour procedural over substantive
approaches where systems must serve plural moralities: the reflective
procedure improved judgment without committing the model to a
particular value position. For evaluation methodology, moral-benchmark
scores should be decomposed — by error direction, human-vote
alignment, and response compliance — before differences between
value-conditioned systems are interpreted as differences in ethical
competence, and verdict extraction itself requires validation, since
value prompts alter response style enough to produce spurious effects.
Two extensions follow directly from this design: testing whether
vow-conditioned models track annotators from Buddhist cultural spheres,
and testing whether the content/procedure dissociation survives
fine-tuning.
# Declarations

**Funding.** Reported on the title page.

**Conflicts of interest.** The authors declare no competing interests.

**Ethics approval.** Not applicable. The study involved no human
participants; all human judgments analysed are from previously published,
publicly released datasets (ETHICS, MoralChoice, SCRUPLES, MMLU) used
under their respective licences.

**Data availability.** All code, prompt texts, the answer-extraction
test suite, and the complete per-item records (27,352 judgments,
including raw model responses) are publicly available; the repository
URL is withheld here for double-anonymous review and is provided on the
title page. Every experiment runs on consumer hardware with open-weight
models and fixed seeds at temperature 0.

**AI use disclosure.** The authors used an AI coding assistant for
experiment harness implementation, data analysis scripting, and
manuscript drafting under author direction; all study design decisions,
doctrinal interpretations, and final text were reviewed and approved by
the authors. Language models under study (Qwen 3.6, Gemma 3, GPT-OSS,
Llama 3.3) are the objects of the research, not authors of it.

**Author contributions.** Reported on the title page.
# References (APA 7)
...
Costa, D. B., Alves, F., & Vicente, R. (2025). *Moral susceptibility and
robustness under persona role-play in large language models*. arXiv.
https://arxiv.org/abs/2511.08565

Dayal, H. (1932). *The bodhisattva doctrine in Buddhist Sanskrit
...
Hongladarom, S. (2020). *The ethics of AI and robotics: A Buddhist
viewpoint*. Lexington Books.

Huang, A., Pi, Y. N., & Mougan, C. (2024). *Moral persuasion in large
language models: Evaluating susceptibility and ethical alignment*.
arXiv. https://arxiv.org/abs/2411.11731

Lourie, N., Le Bras, R., & Choi, Y. (2021). SCRUPLES: A corpus of
community ethical judgments on 32,000 real-life anecdotes. *Proceedings
of the AAAI Conference on Artificial Intelligence, 35*(15),
13470-13479. https://ojs.aaai.org/index.php/AAAI/article/view/17589

Madaan, A., Tandon, N., Gupta, P., Hallinan, S., Gao, L., Wiegreffe, S.,
... Clark, P. (2023). Self-Refine: Iterative refinement with
...
large language models. *Proceedings of the 61st Annual Meeting of the
Association for Computational Linguistics*, 428-446.

Santurkar, S., Durmus, E., Ladhak, F., Lee, C., Liang, P., & Hashimoto,
T. (2023). Whose opinions do language models reflect? *Proceedings of
the 40th International Conference on Machine Learning*.

Scherrer, N., Shi, C., Feder, A., & Blei, D. (2023). Evaluating the
moral beliefs encoded in LLMs. *Advances in Neural Information
...
*Proceedings of the 29th Annual Meeting of the Association for Natural
Language Processing (NLP2023)*.

Thomas, R. S., Shiromani, S., Chaudhry, A., Li, R., Sharma, V., Zhu, K.,
& Dev, S. (2026). *ProMoral-Bench: Evaluating prompting strategies for
moral reasoning and safety in LLMs*. arXiv.
https://arxiv.org/abs/2602.13274

Vallor, S. (2016). *Technology and the virtues: A philosophical guide
